(function () {
    ...
    var root = document.querySelector('[data-storage-access-gate-root]');
    var form = document.getElementById('storage-access-gate-form');
    if (!root || !form) {
        return;
    }

    var pointerField = document.getElementById('storage-access-gate-pointer-moves');
    var focusField = document.getElementById('storage-access-gate-focus-events');
    var timeField = document.getElementById('storage-access-gate-time-on-page');
    var webdriverField = document.getElementById('storage-access-gate-webdriver');
    var refreshButton = document.getElementById('storage-access-gate-captcha-refresh');
    var captchaImage = document.querySelector('.storage-access-gate__captcha-image');
    var captchaInput = document.getElementById('captcha-code');
    var phasePanels = root.querySelectorAll('[data-storage-access-gate-phase]');
    ...
    function setPhase(phase) {
        ...
        phasePanels.forEach(function (panel) {
            var isActive = panel.getAttribute('data-storage-access-gate-phase') === phase;
            panel.classList.toggle('d-none', !isActive);
            panel.classList.toggle('storage-access-gate__phase--active', isActive);
        });
    }
    ...
    function isAccessGateRedirect(location) {
        ...
        try {
            ...
            return url.pathname.indexOf('/access-gate') === 0;
        } catch (error) {
            return location.indexOf('/access-gate') >= 0;
        }
    }
